...
class SuperGPETokenizer:
    ...
    def _encode_no_merge(self, text):
        text_chunks = regex.findall(self.whitespace_pattern, text)
        text_chunks = [t.replace(' ','\u2581') for t in text_chunks if t.strip()]
        ...
        for chunk in text_chunks:
            graphemes_list = list(grapheme.graphemes(chunk))
            for g in graphemes_list:
                ids.append(self.vocab_re.get(g, self.unk_token_id))
        return ids

    def _apply_merges_subset(self, ids, merges_subset):
        # merges_subset = [(pair, idx), ...] in order
        for pair, idx in merges_subset:
            ids = self._merge(ids, pair, idx)
        return ids

    def extend(self, texts, inherit=28000, vocab=32000):
        """
        Extend a trained GPE tokenizer:
        - inherit first `inherit` vocab
        - continue merging up to `vocab`
        """

        if not self.trained:
            raise ValueError("Tokenizer must be trained or loaded before extend()")

        print(f"Extending tokenizer: inherit={inherit} → vocab={vocab}")

        # --- Step 1: get ordered merges ---
        ordered_merges = list(self.merges.items())  # preserves order

        # --- Step 2: keep only merges that produce ids < inherit ---
        kept_merges = [(pair, idx) for pair, idx in ordered_merges if idx < inherit]

        # --- Step 3: rebuild vocab (truncate) ---
        new_vocab = {}
        new_vocab_re = {}

        for i in range(inherit):
            token = self.vocab[i]
            new_vocab[i] = token
            new_vocab_re[token] = i

        # rebuild merges dict (same structure as before)
        new_merges = {pair: idx for pair, idx in kept_merges}

        print(f"Inherited vocab size: {len(new_vocab)}")
        print(f"Inherited merges: {len(new_merges)}")

        # --- Step 4: prepare training data ---
        ids_list = []

        for text in tqdm(texts, desc="Encoding for extension"):
            ids = self._encode_no_merge(text)
            ids = self._apply_merges_subset(ids, kept_merges)
            if ids:
                ids_list.append(ids)

        # --- Step 5: continue merging ---
        current_vocab_size = inherit
        target_vocab_size = vocab
        num_new_merges = target_vocab_size - current_vocab_size

        print(f"Running {num_new_merges} new merges...")

        for i in tqdm(range(num_new_merges), desc="SuperGPE merging"):
            stats = {}
            ...
            # update structures
            new_merges[best_pair] = new_id
            new_vocab[new_id] = new_vocab[best_pair[0]] + new_vocab[best_pair[1]]
            new_vocab_re[new_vocab[new_id]] = new_id

            if i % 500 == 0:
                tqdm.write(f"[{i}] merge: {new_vocab[best_pair[0]]} + {new_vocab[best_pair[1]]}")

            current_vocab_size += 1

        # --- Step 6: replace tokenizer state ---
        self.vocab = new_vocab
        self.vocab_re = new_vocab_re
        self.merges = new_merges
        self.vocab_size = len(new_vocab)

        print(f"Extension complete. Final vocab size: {self.vocab_size}")
